...
const Card = (props) => {
  ...
  const ciudadProyectoUrl = replaceSpaces(
    proyecto?.comunaNombre,
  )?.toLowerCase();
  const nombreProyectoUrl = replaceSpaces(
    proyecto?.nombreWebProyecto,
  )?.toLowerCase();
  const textColorClass =
    proyecto.colorEtapa === "--malpo-paleta-de-colores-blanco"
      ? "text-black"
      : "text-white";
  ...
  return (
    <>
      {modelos.map((modelo, index) => {
        const modeloNombreUrl = replaceSpaces(
          modelo.nombreModelo,
        ).toLowerCase();
        const idProyecto = modelo.idProyecto;
        const idModelo = modelo.idModelo;
        const isDisponible = modelo.disponiblidadModelo === "1";

        return (
          <div
            key={index}
            className="relative m-5 flex h-[400px] w-[300px] flex-shrink-0"
          >
            {/* Degradado de fondo */}
            <div className="absolute inset-0 rounded-b-xl bg-gradient-to-b from-gray-950 to-gray-400"></div>
            {isDisponible ? (
              <a
                href={`/proyecto/${nombreProyectoUrl}/${modeloNombreUrl}?val1=${idProyecto}&val2=${idModelo}`}
                className=""
              >
                {/* Imagen de fondo */}
                <picture>
                  ...
                  <div className="mt-4 h-10">
                    <ul className="text-white">
                      <li className="flex items-center">
                        ...
                        <div className="ml-2">
                          Desde UF {formatoNumero(modelo.valorUfModelo)}
                        </div>
                      </li>
                    </ul>
                  </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 rounded-b-xl bg-rojoMalpo px-2 py-2 text-white">
                  <div className="hover:text-gray-400">
                    Ir al Detalle - {modelo.nombreModelo}
                  </div>
                </div>
              </a>
            ) : (
              <div>
                ...
                {/* Footer */}
                <div className="absolute bottom-0 left-0 right-0 rounded-b-xl bg-rojoMalpo px-2 py-2 text-white">
                  <div className="hover:text-gray-400">Sin Stock</div>
                </div>
              </div>
            )}
          </div>
