...
export default async function Proyecto({
  ...
  const dividendo = formatNumberWithThousandSeparator(
    parseInt(proyectoInvertir?.dividendo),
  );
  const arriendo = formatNumberWithThousandSeparator(
    parseInt(proyectoInvertir?.arriendo),
  );
  const imagenCabecera =
    proyectoInversionistaData?.datos?.proyecto?.imagenCabecera;
  const imagenMiniatura =
    proyectoInversionistaData?.datos?.proyecto.imagenMiniatura;
  const urlEficienciaEnergetica =
    proyectoInversionistaData?.datos?.recursos?.idTipo == "2"
  ...
  const url = (
    <UrlBanner
      nombre={
        nombre
      } /* idRegion={proyectoInversionistaData?.datos?.proyecto?.idRegion} nombreRegion={proyectoInversionistaData?.datos?.proyecto?.regionNombre} idComuna={proyectoInversionistaData?.datos?.proyecto?.idComuna} comunaNombre={proyectoInversionistaData?.datos?.proyecto?.comunaNombre} */
    />
  );
  return (
    <>
      ...
      <div className="mx-auto w-11/12 pb-3 pt-3 md:w-10/12">
